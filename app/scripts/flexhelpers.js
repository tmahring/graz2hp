...
.directive('lightboxGallery', function(Fullscreen) {
  return {
    ...
    controller: function($scope) {
      ...
      this.addImage = function(thumb, full) {
        $scope.addImage(thumb, full);
      };
    },
    link: function($scope, $element, $attr) {
      ...
      $scope.images = [];
      $scope.addImage = function(thumb, full) {
        $scope.images.push({
          thumb: thumb,
          full: full
        });
      ...
      var nextImg, prevImg;
      $scope.$watch('imgFull', function() {
        for(var i = 0; i < $scope.images.length; i++) {
          if($scope.images[i].full === $scope.imgFull) {
            console.log(i, $scope.images.length);
            if(i < ($scope.images.length - 1)) {
              $scope.hasNext = true;
              nextImg = $scope.images[1+i].full;
            }
            else {
              $scope.hasNext = false;
            }
      ...
      $scope.next = function() {
        $scope.imgFull = nextImg;
      };
      $scope.prev = function() {
        $scope.imgFull =prevImg;
      };

      Fullscreen.$on('FBFullscreen.change', function(evt, isFullscreenEnabled){
        console.log('FULLSCREEN:' + isFullscreenEnabled);
        $scope.showMe = isFullscreenEnabled;
      });
    },
  };
});
